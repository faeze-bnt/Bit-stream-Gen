...
for i = 1:length(Stream_Length_list)
    ...
    for j = 1:length(Parallel_deg_list)
        ...
        for ii = 2:x-1
            for jj = 2:y-1
                In =   [double(image_input(ii-1, jj-1))/256,
                        double(image_input(ii-1, jj))/256,
                        double(image_input(ii-1, jj+1))/256,
                        double(image_input(ii, jj-1))/256,
                        double(image_input(ii, jj))/256,
                        double(image_input(ii, jj+1))/256,
                        double(image_input(ii+1, jj-1))/256,
                        double(image_input(ii+1, jj))/256,
                        double(image_input(ii+1, jj+1))/256];
